...
class BCO():
 ...
 5.57054799e-02, 2.21830219e-04, 2.05036859e-04, 3.07063172e-10]
    ...
    def D_demo(self, states, nstates):
        """
        State and next state normalization with min-max
        """
    ...
    def logit_normalization(self, x_min_arr, x_max_arr, logits):
        """
        Normalize each feature of each logit with min-max normalization 
        
        x_min = tf.constant(x_min_arr, dtype=logits.dtype)
        x_max = tf.constant(x_max_arr, dtype=logits.dtype)

        norm_logits = (logits - x_min) / (x_max - x_min)
        """
    ...
    def action_unormalization(self, a_min, a_max, plc_action):
        """
        Unormalize the action according to expert's min-max
        ...
        for i in range(len(plc_action)):
            ...
            un_feature = feature * (x_max - x_min) + x_min
    ...
    def policy(self):
        ...
        for dim in range(self.action_dim):
            plc_loss = tf.reduce_mean(tf.keras.losses.MeanSquaredError()(self.action[:, dim], self.plc_action[:, dim]))
            plc_losses.append(plc_loss)

            plc_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate)
            plc_train_step = plc_optimizer.minimize(plc_loss)
            plc_train_steps.append(plc_train_step)

        self.plc_losses = plc_losses
        self.plc_train_steps = plc_train_steps
        """
    ...
    def inverse_dynamic_model(self):
        ...
        """
        for dim in range(self.action_dim):
            idm_loss = tf.reduce_mean(tf.keras.losses.MeanSquaredError()(self.action[:, dim], self.idm_action[:, dim]))
            idm_losses.append(idm_loss)

            idm_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate)
            idm_train_step = idm_optimizer.minimize(idm_loss)
            idm_train_steps.append(idm_train_step)

        self.idm_losses = idm_losses
        self.idm_train_steps = idm_train_steps
        """
    ...
    def state_normalization(self, x_min_arr, x_max_arr, state):
        ...
        for i in range(len(x_min_arr)):
            ...
            x_max = x_max_arr[i]
            feature = state[i]
            feature_normalized = (feature - x_min)/(x_max - x_min) 
